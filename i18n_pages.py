...
def header(lang,current,prefix='../'):
    ...
    return f'<header class="site-header"><div class="shell nav-wrap"><a class="brand" href="/{d["base"]}/"><img src="/assets/logo-tf.png" alt="Traços Fidalgos"><span>Traços Fidalgos</span></a><button class="nav-toggle" aria-label="Menu">☰</button><nav class="nav"><a href="/{d["base"]}/{d["atelier"]}/">{nav["atelier"]}</a><a href="/{d["base"]}/{d["services"]}/">{nav["services"]}</a><a href="/{d["base"]}/{d["catalogues"]}/">{nav["portfolio"]}</a><a href="/{d["base"]}/{d["process"]}/">{nav["process"]}</a><a href="/{d["base"]}/paris/">{nav["paris"]}</a><a href="/{d["base"]}/{d["contact"]}/">{nav["contact"]}</a><a class="pill" href="/{d["base"]}/{d["contact"]}/">{nav["quote"]}</a><span class="lang"><a href="{maps["pt"]}">PT</a><a href="{maps["en"]}">EN</a><a href="{maps["fr"]}">FR</a></span></nav></div></header>'
